...
1. 不要使用通配符导入（`*`），必须显式导入具体类。
2. 避免创建不必要的对象。
3. 应复用已有对象，如 `str1` / `str2` 在声明后，可以日志输出 `str1` 和 `str2` 的值，避免被 codecheck 工具检测为无用对象。
4. `import` 语句必须按以下顺序分组，并用空行分隔：
    - Android 包
    - 公司内部包
    - 商业第三方库
    - 开源库（`net` / `org`）
    - Java 标准库（`java` / `javax`）
5. 不要使用构造函数创建 BigDecimal、BigInteger 以及基本类型包装类对象（String，Char 等八个基本类型对应的包装对象），应使用工厂方法（如 `valueOf`）。
6. 优先使用泛型集合而不是数组。
7. 对最多只执行一次的循环进行重构：
    - 如果测试因子包含循环，则保证循环执行一次以上；
    - 如果不包含循环，则重构为简单条件判断语句。
8. 避免无意义的布尔表达式，表达式必须具备明确逻辑意义。
9. 保证生成的代码符合 Java 代码风格规范，不要有多余或缺少的空格和空行。
